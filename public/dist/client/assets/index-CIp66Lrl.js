import { m as motionValue, u as useIsomorphicLayoutEffect$1, a as addDomEvent, b as bindModuleMethods, g as getDocument, $, c as getWindow, n as now, d as nextTick, e as getTranslate, f as classesToSelector, h as extend, i as isObject, A as Autoplay, N as Navigation, P as Pagination, S as Scrollbar, j as Swiper, V as Virtual, k as jsx, l as jsxs, F as FaTimes, L as Link, r as reactExports, o as motion, p as getAugmentedNamespace, q as requireReact, s as Fragment, t as AnimatePresence, O as OUR_SERVICES_HASH_LINK, B as BsDot, v as Button, w as getImageALt, x as FaArrowCircleRight, y as BsArrowRight, z as useIntl } from "./index-CG8AyQ8c.js";
...
function requireUseIsomorphicLayoutEffect() {
  ...
  useIsomorphicLayoutEffect.useIsomorphicLayoutEffect = useIsomorphicLayoutEffect$12;
  var _react = requireReact();
  function useIsomorphicLayoutEffect$12(callback, deps) {
...
const Tablet = () => {
  const [slideForTablet, setSlideForTablet] = reactExports.useState(0);
  const [rendered, setRendered] = reactExports.useState(false);
  reactExports.useEffect(() => {
    setRendered(true);
  }, []);
...
const Mobile = () => {
  const [slideForMobile, setSlideForMobile] = reactExports.useState(0);
  const [rendered, setRendered] = reactExports.useState(false);
  reactExports.useEffect(() => {
    setRendered(true);
  }, []);
...
const HomePageServices = () => {
  const refOne = reactExports.useRef(null);
  const lengthRef = reactExports.useRef(null);
  const [scrolled, setScrolled] = reactExports.useState(false);
  const [number, setNumber] = reactExports.useState(0);
  const {
    scrollY
  } = useViewportScroll();
...
const Consultation = () => {
  ...
  const refLast = reactExports.useRef(null);
  reactExports.useEffect(() => {
